...
load_test_data(const char *testpath, size_t o, size_t v, int is_upt,
    ...
    double *i_oovv, const char *i_ovvv_path)
{
	FILE *fp;
	size_t i, j;
	...
	double *vvv = xmalloc(vvv_bytes);
	int fd;
	if ((fd = open(i_ovvv_path, O_WRONLY|O_CREAT, S_IRUSR|S_IWUSR)) == -1)
		err(1, "open");
	for (i = 0; i < o; i++) {
		for (j = 0; j < v*v*v; j++)
			vvv[j] = read_next_double(fp);
		if (write(fd, vvv, vvv_bytes) != vvv_bytes)
			err(1, "write");
	}
	if (!is_upt) {
		skip_line(fp);
...
main(int argc, char **argv)
{
	...
	double *d_ov, *f_ov, *t1, *t2, *i_oovv, *i_oovo;
	const char *errstr, *testpath = NULL, *i_ovvv_path = "i_ovvv";
	int is_upt = 0, rank;
	char ch;
	...
	if (testpath) {
		load_test_data(testpath, o, v, is_upt, d_ov, f_ov,
		    t1, t2, i_oovo, i_oovv, i_ovvv_path);
	} else {
...
//		    i_oovo, i_oovv, i_ovvv_path);
	} else {
		e_pt = ccsd_pt_disk(o, v, d_ov, f_ov, t1, t2,
		    i_oovo, i_oovv, i_ovvv_path);
	}
	if (rank == 0) {
		time_t t = time(NULL);
