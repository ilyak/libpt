...
main(int argc, char **argv)
{
	...
	if ((d_ov = malloc(d_ov_sz * sizeof(double))) == NULL)
		err(1, "malloc");
	if ((f2_ov = malloc(d_ov_sz * sizeof(double))) == NULL)
		err(1, "malloc");
	...
	read_test_data(fp, oa, va, ob, vb, d_ov, f2_ov, l1, t2, l2, i_oovv,
	    i2_t2f2_oovo, i3_ovvv, i6_oovo, i7_ovvv);
	fclose(fp);

	e_cmp = libpt_uft(oa, va, ob, vb, d_ov, f2_ov, l1, t2, l2, i_oovv,
	    i2_t2f2_oovo, i3_ovvv, i6_oovo, i7_ovvv);
	if (rank == 0) {
